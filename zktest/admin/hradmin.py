...
class LeaveBalanceInline(TabularInline):
    model = LeaveBalance
    extra = 0
    fields = ('leave_type_code', 'year', 'entitled_days', 'used_days', 'carried_forward_days')
    readonly_fields = ()
    show_change_link = True
    tab = True
...
class AttendanceAdmin(ModelAdmin):
    list_display = (
        'user_id', 'date', 'display_status', 'shift_code',
...
class LeaveApplicationAdmin(ModelAdmin):
    list_display = (
        'user_id', 'leave_type_code', 'start_date', 'end_date', 
...
class UserLocationAdmin(ModelAdmin):
    list_display = ('user_id', 'location', 'is_primary', 'created_at')
    list_filter = (
        'is_primary',
        ('location', RelatedDropdownFilter),
    )
    search_fields = ('user_id__first_name', 'user_id__last_name', 'location__name')
    ordering = ('user_id', 'location')
    list_per_page = 50
...
class RosterAssignmentAdmin(ModelAdmin):
    list_display = ('roster', 'user_id', 'shift_code', 'created_at')
    list_filter = (
...
class RosterDayAdmin(ModelAdmin):
    list_display = ('user_id', 'date', 'shift_code', 'is_off', 'created_at')
    list_filter = (
        'is_off',
        ('date', RangeDateFilter),
        'shift_code',
