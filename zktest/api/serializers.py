...
class ZKTecoAttendanceSerializer(serializers.Serializer):
    """Serializer for ZKTeco device attendance push data"""
    ...
    table = serializers.CharField(required=False, help_text="Table name for POST data")
    
    def create(self, validated_data):
        """Process ZKTeco attendance data"""
        sn = validated_data.get('SN', '')
        attlog = validated_data.get('ATTLOG', '')
        
        if attlog:
            # Parse GET method data
            parts = attlog.split(",")
            if len(parts) >= 2:
                user_id = parts[0]
...
class ZKTecoPostDataSerializer(serializers.Serializer):
    """Serializer for ZKTeco device POST data"""
